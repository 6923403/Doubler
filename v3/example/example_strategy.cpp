...
public:
    User(string n, int c, Strategy* s) : name(n), coins(c), strategy(s) {} // 构造函数，初始化用户名，硬币和策略
...
public:
    ...
    virtual void calculateReward(User* u, POOL* p) = 0; // 计算奖励的纯虚函数
};

// 具体策略类，按照比例分发奖励
class ProportionStrategy : public Strategy {
public:
...
// 具体策略类，按照排名分发奖励
class RankingStrategy : public Strategy {
public:
    void calculateReward(User* u, POOL* p) override {
        // 对用户按照投入的硬币降序排序
...
int main() {
    ...
    // 创建两个策略对象
    Strategy* proportionStrategy = new ProportionStrategy();
    Strategy* rankingStrategy = new RankingStrategy();
    // 创建三个用户，分别使用不同的策略
